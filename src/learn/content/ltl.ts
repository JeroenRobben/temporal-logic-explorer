...
export const TUT_LTL_X: Tutorial = {
  ...
  steps: [
    ...
    { text: 'A ✗ LTL verdict always comes with a witness path. Click **Load counterexample** on the ✗ `X p` row — the lasso s0 → s2 (looping at s2) loads as a trace, and the timeline matrix shows `X p` ✗ at position 0.',
      checkpoint: hasTrace(),
...
export const TUT_LTL_F: Tutorial = {
  ...
  steps: [
    ...
    { text: 'Click **Load counterexample** on the ✗ row. The lasso s0 ⇄ s1 (looping back to s0) visits `r` at every s1 — infinitely often! — yet never *stays* in r: each return to s0 breaks `G r` again.',
      checkpoint: hasTrace(),
...
export const TUT_LTL_G: Tutorial = {
  ...
  steps: [
    ...
    { text: 'Click **Load counterexample** on the ✗ row: the lasso s0 → s2 (staying at s2) visits `s2` once — and one q-less position is all it takes. G is as strong as its weakest state.',
      checkpoint: hasTrace(),
      solution: { trace: { stateIds: ['s0', 's2'], loopIndex: 1 } } },
...
export const TUT_LTL_U: Tutorial = {
  ...
  steps: [
    ...
    { text: 'Click **Load counterexample** on the ✗ row to see a concrete failing path: s0 → s2 (staying at s2). The timeline matrix shows `!p` already false at position 0, before `r` ever arrives.',
      checkpoint: hasTrace(),
      solution: { trace: { stateIds: ['s0', 's2'], loopIndex: 1 } } },
